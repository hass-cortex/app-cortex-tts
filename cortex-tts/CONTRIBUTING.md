...
**A word is read the mainland way and is not in the readings table.** The
table (`src/cortex_speech/text/zh/taiwan_readings.tsv`) is generated by
`scripts/taiwan_readings.py` from the McBopomofo dictionary; edit the script's
rules and rerun `uv run scripts/taiwan_readings.py`, never the file. A word is
missing for one of three reasons the script prints: the dictionary never saw
it in its corpus, its reading is contextual and the dictionary's default did
not settle it, or no character reads its syllable the Taiwan way by default on
both sides.
