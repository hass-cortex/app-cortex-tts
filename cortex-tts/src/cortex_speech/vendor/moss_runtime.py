"""MOSS-TTS-Nano ONNX runtime, vendored from OpenMOSS/MOSS-TTS-Nano @ 8b7bcc9.

Kept close to upstream so a diff stays readable. Four deviations, all of them
removing a dependency this app does not want in the image:

1. `torch` / `torchaudio` are gone. They were imported at module scope for one
...
2. `text_normalization_pipeline` is gone. It pulls WeTextProcessing and pynini
   ...
   normalisers disagreeing. `prepare_synthesis_text` is now the identity path:
   the app hands this runtime text that is already prepared.
3. `moss_tts_nano.defaults` is gone; nothing here writes to a default output
   path, because the engine always asks for the waveform in memory.
5. `synthesize_single_chunk` takes an `on_audio_chunk` callback. Upstream
   decodes incrementally and then returns one waveform, so the chunks it had
   already produced could not be observed. Nothing else about the loop changes.

4. `DEFAULT_OUTPUT_DIR` is defined here instead of imported. The download and
   WAV-writing helpers it serves are unused — the app fetches bundles itself
   and asks for the waveform in memory — but they stay, because deleting dead
   upstream code is what makes the next diff hard to read.

Everything else is upstream's. Do not tidy it.
"""
...
# Upstream imports this from `moss_tts_nano.defaults`, a package this app does
# not vendor. Nothing reaches it: every synthesis asks for the waveform back
# rather than a file.
DEFAULT_OUTPUT_DIR = Path("/tmp/cortex-tts-moss")

APP_DIR = Path(__file__).resolve().parent
REPO_ROOT = APP_DIR
from .moss_ort import (
    OrtCpuRuntime,
...
DEFAULT_BROWSER_ONNX_TTS_DIR = (
    DEFAULT_BROWSER_ONNX_MODEL_DIR / "MOSS-TTS-Nano-100M-ONNX"
)
DEFAULT_BROWSER_ONNX_CODEC_DIR = (
    DEFAULT_BROWSER_ONNX_MODEL_DIR / "MOSS-Audio-Tokenizer-Nano-ONNX"
)
...
DEFAULT_BROWSER_ONNX_TTS_REPO_URL = (
    f"https://huggingface.co/{DEFAULT_BROWSER_ONNX_TTS_REPO_ID}"
)
DEFAULT_BROWSER_ONNX_CODEC_REPO_URL = (
    f"https://huggingface.co/{DEFAULT_BROWSER_ONNX_CODEC_REPO_ID}"
)
...
def _default_model_dir_requested(model_dir: str | Path | None) -> bool:
    if model_dir is None:
        return True
    return (
        _resolve_model_dir_path(model_dir)
        == DEFAULT_BROWSER_ONNX_MODEL_DIR.expanduser().resolve()
    )
...
def _download_default_browser_onnx_assets(model_dir: Path) -> None:
    logging.info(
        "browser_onnx assets missing under %s; downloading from Hugging Face.",
        model_dir,
    )
...
def ensure_browser_onnx_model_dir(model_dir: str | Path | None = None) -> Path:
    ...
    if not _default_model_dir_requested(model_dir):
        tried_paths = [
            str((resolved_model_dir / item).resolve())
            for item in MODEL_MANIFEST_CANDIDATE_RELATIVE_PATHS
        ]
        raise FileNotFoundError(
            "browser_onnx model assets not found under the provided --model-dir. tried: "
            + ", ".join(tried_paths)
        )
...
class OnnxTtsRuntime(OrtCpuRuntime):
    def __init__(
        ...
        *,
        ...
        tokenizer_relative_path = str(
            self.manifest["model_files"].get("tokenizer_model", "tokenizer.model")
        )
    ...
    def _ensure_text_normalizer(self, enable_wetext: bool) -> None:
        # DEVIATION: upstream builds a WeTextProcessing manager here. See the
        # module docstring — this app normalises text before it ever reaches
        # the runtime, so there is nothing to prepare. Kept as a no-op because
        # upstream's own entrypoints call it during startup.
        del enable_wetext
        return None
    ...
    def prepare_synthesis_text(
        ...
        *,
        ...
        # DEVIATION: upstream normalises here with WeTextProcessing (pynini).
        # This app has its own text path — Traditional-to-Simplified conversion
        # and numeral expansion — and running both would be two normalisers
        # disagreeing about the same string. Text arrives already prepared, so
        # this is the identity path and the flags are accepted and ignored.
        del enable_wetext, enable_normalize_tts_text
        final_text = str(text or "")
        final_prompt_text = str(prompt_text or "")
        return {
            "text": final_text,
    ...
    def _load_reference_audio(self, reference_audio_path: str | Path) -> np.ndarray:
        # DEVIATION: upstream uses torchaudio here, which is the only reason
        ...
        return decoded[np.newaxis, ...].astype(np.float32, copy=False)
    ...
    def encode_reference_audio(
        ...
        output_names = [
            output.name for output in self.sessions["codec_encode"].get_outputs()
        ]
    ...
    def synthesize_single_chunk(
        ...
        *,
        ...
        # DEVIATION: `on_audio_chunk` is ours. Upstream decodes incrementally
        # and then hands back one waveform, so the chunks it already produced
        # are unreachable. The callback publishes each as it appears, which is
        # what time-to-first-audio depends on.
        ...
        def decode_pending_frames(force: bool) -> None:
            ...
            if not force and pending_count < max(1, decode_budget):
                return
            frame_budget = (
                pending_count if force else min(pending_count, max(1, decode_budget))
            )
        ...
        try:
            generated_frames = self.generate_audio_frames(
                request_rows, on_frame=on_frame
            )
    ...
    def synthesize(
        ...
        *,
        ...
        self.manifest["generation_defaults"]["do_sample"] = (
            normalized_sample_mode != SAMPLE_MODE_GREEDY
        )
        if seed is not None:
        ...
        audio_path = _write_waveform_to_wav(
            resolved_output_audio_path, waveform, sample_rate
        )
        return {
