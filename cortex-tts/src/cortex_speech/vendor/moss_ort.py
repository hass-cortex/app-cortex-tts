...
def _resolve_stream_decode_frame_budget(
    ...
    lead_seconds = _compute_stream_lead_seconds(
        emitted_samples_total, sample_rate, first_audio_emitted_at_seconds
    )
...
class OrtCpuRuntime:
    def __init__(
        ...
        self.manifest["generation_defaults"]["sample_mode"] = _normalize_sample_mode(
            ...
            else self.manifest["generation_defaults"].get("sample_mode"),
        ...
        self.tts_meta_path = self.resolve_manifest_relative_path(
            manifest["model_files"]["tts_meta"]
        )
        self.codec_meta_path = self.resolve_manifest_relative_path(
    ...
    def _create_sessions(self) -> dict[str, ort.InferenceSession]:
        ...
        return {
            ...
            "local_decoder": self._session(
                tts_dir / self.tts_meta["files"]["local_decoder"]
            ),
            **(
                {
                    "local_greedy_frame": self._session(
                        tts_dir / self.tts_meta["files"]["local_greedy_frame"]
                    )
                }
            ...
            **(
                {
                    "local_cached_step": self._session(
                        tts_dir / self.tts_meta["files"]["local_cached_step"]
                    )
                }
            ...
            "codec_encode": self._session(
                codec_dir / self.codec_meta["files"]["encode"]
            ),
            "codec_decode": self._session(
                codec_dir / self.codec_meta["files"]["decode_full"]
            ),
    ...
    def warmup(self) -> None:
        ...
        request_rows = self.build_voice_clone_request_rows(
            voice["prompt_audio_codes"], text_sample["text_token_ids"]
        )
        ...
        if (
            ...
            and self.manifest["generation_defaults"]["sample_mode"] == SAMPLE_MODE_FIXED
        ):
        ...
        elif "local_greedy_frame" in self.sessions and not bool(
            self.manifest["generation_defaults"]["do_sample"]
        ):
    ...
    def create_empty_local_cached_past(self) -> dict[str, np.ndarray]:
        ...
        return {
            ...
            for name in (
                f"local_past_key_{layer_index}",
                f"local_past_value_{layer_index}",
            )
        }
        ...
        *,
        ...
        outputs = self.sessions["local_cached_step"].run(
            None,
            {
                ...
                "past_valid_lengths": np.asarray(
                    [int(past_valid_lengths)], dtype=np.int32
                ),
                **local_past_by_name,
            },
            self.run_options,
        ...
        named_outputs = dict(zip(output_names, outputs, strict=True))
        next_local_past = {
            output_name.replace("local_present_", "local_past_"): named_outputs[
                output_name
            ]
            for output_name in self.tts_meta["onnx"]["local_cached_output_names"][2:]
        }
        return (
            named_outputs["text_logits"].reshape(-1),
            named_outputs["audio_logits"],
            next_local_past,
        )
    ...
    def slice_audio_channel_logits(
