// Synthesis: hand the composer's text to the model and play what comes back.

import { call } from "./api.js";
import { $, msg, pressed } from "./dom.js";
import { play, showStats } from "./player.js";
import { refreshModels } from "./models.js";

async function speak() {
  $("speak").disabled = true;
  msg($("speakMsg"), "");
  try {
    const res = await call("/speak", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        text: $("text").value,
        model: $("model").value,
        voice: $("voice").value,
        normalize_text: pressed($("norm")),
        convert_script: pressed($("conv")),
      }),
    });
    play(URL.createObjectURL(await res.blob()), { revokable: true });

    const seconds = Number(res.headers.get("X-Hojo-Audio-Seconds") || 0);
    const ms = Number(res.headers.get("X-Hojo-Inference-Ms") || 0);
    showStats([
      ["audio", `${seconds.toFixed(2)}s`],
      ["inference", `${Math.round(ms)}ms`],
      ["RTF", res.headers.get("X-Hojo-Rtf") || "—"],
      ["segments", res.headers.get("X-Hojo-Segments") || "—"],
    ]);
    // Synthesis loads and evicts models, so the table has just gone stale.
    await refreshModels();
  } catch (err) {
    msg($("speakMsg"), err.message, "err");
  } finally {
    $("speak").disabled = false;
  }
}

export function init() {
  $("speak").addEventListener("click", speak);
}
