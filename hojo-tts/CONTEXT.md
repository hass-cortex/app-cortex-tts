...
`CloneEngine` for the 80M). One engine serves one synthesis at a time: the
...
`/api/speak` per sentence; the app's own HTTP response is never chunked.
...
- A **Voice** belongs to exactly one model. On the 80M it _is_ a **Reference
  recording**, so uploading one creates a voice and deleting one destroys it.
...
- **"streaming" is three things.** Home Assistant's _streaming input_ (the
  ...
  synthesis** (one `/api/speak` per sentence), and HTTP chunked responses
  (which this app does not do). A sensor that measured the first of these was
  removed precisely because the name promised one and the clock measured
  another.
- **"voice" without a model is meaningless.** `hojo_zh_f_01` exists on the 40M
  and nowhere else; the 80M's voices are whatever references have been
  uploaded. Always say which model.
...
- **"default_voice"** is a configured _name_, not a resolved voice. It is
